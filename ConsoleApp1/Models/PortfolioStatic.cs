...
namespace C1Finance.Models
{
    ...
    public static class PortfolioStatic
    {
        ...
        public partial class SymbolPriceHistory
        {
            ...
            //Class Constructor
            public SymbolPriceHistory(DateTime P_PriceDate, string P_TimeSlabStr, double P_Price, double P_PriceGrowth)
            {
            ...
            if (PortfolioList == null || PortfolioList.Count == 0)
            {
                lock (_lockPortfolioListObj)
                {
                    if (PortfolioList == null || PortfolioList.Count == 0)
                    {
                        ...
                        PriceHistory = new List<SymbolPriceHistory>();
                        Price = 3474;
                        GetPrices("MSFT", Price, out LastPrice, out LastPrice2, out PriceHistory);
                        if (LastPrice2 > 0)
                            Change = (LastPrice - LastPrice2) / LastPrice2;
                        Shares = 56;
                        Cost = Price * Shares;
                        ...
                        if (Price > 0)
                            Gain = (LastPrice - Price) / Price;
                        if (!PortfolioList.Exists(p => p.Symbol == "MSFT"))
                            PortfolioList.Add(new Portfolio(CompanyCounter, Company_Palette[CompanyCounter % Company_Palette.Length], SymbolList.Where(x => x.symbol == "MSFT").Select(x => x.name).FirstOrDefault(), "MSFT", true, LastPrice, Change, Shares, Price, Cost, Value, Gain, PriceHistory));
                    }
                }
            }
            ...
            try
            {
                using (StreamReader sr = new StreamReader(dataStream))
                {
                    ...
                    for (var line = sr.ReadLine(); line != null; line = sr.ReadLine())
                    {
                        ...
                        DateTime timeslab = DateTime.Parse(items[0].ToString());
                        //if (!(timeslab >= new DateTime(2016, 01, 14)
                        //    && timeslab <= new DateTime(2016, 07, 13)))
                        //{
                        //    continue;
                        //}

                        if (!(timeslab >= new DateTime(2021, 04, 19)
                            && timeslab <= new DateTime(2021, 10, 18)))
                        {
                            continue;
                        }
                        ...
                        TempList.Add(new SymbolPriceHistory(timeslab, DateTime.Parse(items[0].ToString()).Month.ToString() + "-" + DateTime.Parse(items[0].ToString()).Year.ToString(), value, 0));
                    }
                    ...
                    for (int tcount = TempList.Count - 1; tcount >= 0; tcount--)
                    {
                        ...
                        PriceHistory.Add(new SymbolPriceHistory(TempList[tcount].PriceDate, "", TempList[tcount].Price, pricegrowth));
                    }
                }
